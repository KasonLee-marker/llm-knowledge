...
| 模型 | 发布时间 | 上下文窗口 | 多模态 | 输入价格($/1M tok) | 输出价格($/1M tok) | 最大输出 | 定位 |
|------|---------|-----------|--------|--------------------|--------------------|---------|------|
| Claude 3 Haiku | 2024-03 | 200K | ✅（图像） | $0.25 | $1.25 | 4K | 快速/低成本 |
| Claude 3 Sonnet | 2024-03 | 200K | ✅（图像） | $3.00 | $15.00 | 4K | 均衡 |
| Claude 3 Opus | 2024-03 | 200K | ✅（图像） | $15.00 | $75.00 | 4K | 最强能力 |
| Claude 3.5 Sonnet | 2024-06/10 | 200K | ✅（图像）| $3.00 | $15.00 | 8K | 旗舰首选 ⭐ |
| Claude 3.5 Haiku | 2024-11 | 200K | ✅（图像） | $0.80 | $4.00 | 8K | 高性价比 ⭐ |
| Claude 3.7 Sonnet | 2025-02 | 200K | ✅（图像） | $3.00 | $15.00 | 128K | 推理增强 |
...
## 3. Claude 3.5 Sonnet（当前旗舰，推荐）

### 3.1 核心优势

- **代码能力顶尖**：在 SWE-Bench（真实 GitHub Issues 修复）得分 49%，超越 GPT-4o（38.5%）
- **指令遵循精确**：擅长格式约束、角色扮演、长对话一致性
- **长上下文表现稳定**：200K token 窗口，在长文档检索（Needle-in-Haystack）测试中接近完美
- **Computer Use**：支持操控电脑桌面（Beta 功能）
- **低幻觉率**：Anthropic 的对齐技术使其输出更保守可靠

### 3.2 局限性

- **不支持 Function Calling 的实时流式**（限制部分场景）
- **价格偏高**（与 GPT-4o 相近）
- **图像输入不支持 URL**（需 Base64 或 URL 需确保可访问）
- **无中文特化**（中文能力弱于国产模型）
