...
```mermaid
flowchart TD
    A["开发者注册 Function Schema<br/>+ 用户消息"] --> B

    B{"LLM 推理<br/>是否需要调用函数？"}
    B -- "不需要" --> C["直接生成文本回复 ✅"]
    B -- "需要"   --> D["生成结构化函数调用<br/>name + arguments"]

    D --> E["应用程序执行工具逻辑<br/>（验证参数 → 执行 → 处理异常）"]
    E -- "函数执行结果" --> F["将结果返回给 LLM<br/>LLM 生成最终自然语言回复 ✅"]
